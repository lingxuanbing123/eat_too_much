...
const store = createStore({
  ...
  getters: {
    ...
    cartTotalPrice: state => state.cart.reduce((acc, item) => acc + item.quantity * item.dCost, 0)
  },
  mutations: {
    ADD_TO_CART(state, item) {
      const existingItem = state.cart.find(cartItem => cartItem.commodityName === item.commodityName);
    ...
    UPDATE_CART_ITEM_QUANTITY(state, { commodityName, quantity }) {
      const item = state.cart.find(cartItem => cartItem.commodityName === commodityName);
      if (item) {
